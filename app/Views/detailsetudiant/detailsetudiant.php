	...
	<div class="row">
		<div class="col-lg-6">
			<div class="row">
				<div class="col-lg-12">
					<h1 class="h3"><strong><?= $etudiant['prenom'] ?>
					<?= $etudiant['nom'] ?></strong></h1>
	...
	<div class="row">
		<div class="col-lg-12">
			...
			<?php foreach($commentaire as $valeur): ?>
				...
				$date_inscription = date('d/m/Y H:i:s', strtotime($valeur['date_commentaire']));

				?><br>
				Par <?= $valeur['prenom'] ?> le <?= $date_inscription ?><br>
				<?= $valeur['commentaire'] ?><br>
				<br><br>
			<?php endforeach; ?>
			</div>
			<p>Donner un avis :</p>
			<form method="POST" action="<?= $this->url('add_commentaire').'?id_et='.$etudiant['id_u'];?>">
				<div class="form-group">
					<textarea class="form-control" rows="3" name="commentaire"></textarea>
					<a class="btn btn-default" onclick="postComment(this);" class="btn btn-primary btn-md">Envoyer</a>	
				</div>
			</form>
