...
class UserController extends BaseController
{
	...
	public function updateParticulier()
	{
		...
		if($error == 0)
		{
			...
			$newUser = array('mdp' => $_POST['nouveau_mdp'], 'nom' => $_POST['nom'], 'prenom' => $_POST['prenom'], 'email' => $email,  'statut' => $_POST['statut']);
	...
	public function login()
	{
		...
		{	
			...
			if (!empty($etudiant))
			{
				...
				$connaissance = $newConnaissance->search(['id_et' => $_SESSION['user']['id_u']]);
				...
				$matiere = $newMatiere->search(['nom' => $connaissance['id_m']]);
